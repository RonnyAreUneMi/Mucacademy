...
class EventosView(_AuthenticatedBase):
    ...
    def get(self, request):
        ...
        # Programas publicados, en el mismo feed y marcados como kind='programa'.
        programas = self._programas_feed(request, p, tab)
        results.extend(programas)
        ...
        # Conteos incluyendo programas (los seminarios de programa no cuentan sueltos).
        prog_disp = len(self._programas_feed(request, p, 'disponibles', count_only=True))
        prog_mios = len(self._programas_feed(request, p, 'mios', count_only=True))
        eventos_disp = Event.objects.filter(is_active=True, date__gte=today).filter(VISIBLE_EVENT).exclude(id__in=mis_ids).count()
        eventos_mios = Event.objects.filter(id__in=mis_ids).filter(VISIBLE_EVENT).count()

        return Response({
            'tab': tab,
            'count_mios':        eventos_mios + prog_mios,
            'count_disponibles': eventos_disp + prog_disp,
            'results': results,
        })

    def _programas_feed(self, request, participant, tab, count_only=False):
        """Programas publicados como items del feed (kind='programa').

        En 'disponibles' → no inscritos; en 'mios' → inscritos.
        """
        prog_qs = (Program.objects
        ...
        for prog in prog_qs:
            ...
            if tab == 'disponibles' and is_ins:
                continue
            if tab != 'disponibles' and not is_ins:
                continue
            if count_only:
                items.append(prog.id)
                continue
            cursos = list(prog.active_courses)
            grades, skills = [], []
            for c in cursos:
                ...
                for s in (c.skills or []):
                    if s not in skills:
                        skills.append(s)
            banner = None
            if prog.banner_image:
                banner = request.build_absolute_uri(prog.banner_image.url)
            items.append({
                'kind': 'programa',
                'id': prog.id,
                'title': prog.name,
                'titulo_display': prog.name,
                ...
                'skills': skills,
                'status': 'inscrito' if is_ins else 'disponible',
                'inscrito': is_ins,
            })
        return items
