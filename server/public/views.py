...
def dashboard(request):
    ...
    # Los seminarios de programa no salen sueltos: solo dentro del programa.
    next_event = (Event.objects
        .filter(is_active=True, id__in=eventos_inscrito_ids, date__gte=today, program__isnull=True)
...
def eventos_view(request):
    ...
    if tab == 'disponibles':
        ...
        # Los seminarios de programa no salen sueltos: solo dentro del programa.
        eventos = (Event.objects
            .filter(id__in=mis_ids, program__isnull=True)
            .prefetch_related('speakers')
            .order_by('-date', '-start_time'))
    ...
    # Los programas se muestran en el mismo feed, marcados como "Programa".
    programas_feed = _programs_feed(
        participant=p,
        enrolled=(True if tab != 'disponibles' else False),
    )
    # Programas primero, luego eventos.
    feed = programas_feed + eventos_data

    todos_programas = _programs_feed(participant=p)
    programas_inscrito = sum(1 for it in todos_programas if it['inscrito'])
    programas_disponibles = len(todos_programas) - programas_inscrito

    return render(request, 'public/account/eventos.html', {
        'p': p,
        'tab': tab,
        'eventos_data': feed,
        'count_mios': Event.objects.filter(id__in=mis_ids, program__isnull=True).count() + programas_inscrito,
        'count_disponibles': Event.objects.filter(is_active=True, date__gte=today, program__isnull=True).exclude(id__in=mis_ids).count() + programas_disponibles,
    })
...
def _programs_feed(participant=None, enrolled=None):
    """Programas publicados como items de feed (``type='programa'``).

    ``enrolled``: None → todos · True → solo inscritos · False → solo no inscritos.
    """
    qs = (Program.objects
    ...
    if participant is not None:
        inscrito_ids = set(
            Enrollment.objects
            .filter(participant=participant, event__program__isnull=False)
            .values_list('event__program_id', flat=True)
        )
    feed = []
    for prog in qs:
        is_ins = prog.id in inscrito_ids
        if enrolled is True and not is_ins:
            continue
        if enrolled is False and is_ins:
            continue
        card = _program_card_data(prog)
        card['type'] = 'programa'
        card['inscrito'] = is_ins
        feed.append(card)
    return feed
...
def home(request):
    ...
    # Programas publicados para el landing (mismas tarjetas, sin cuenta).
    current = account_auth.get_current_participant(request)
    programas_grid = _programs_feed(participant=current)

    return render(request, 'public/home.html', {
        'eventos_hero': eventos_hero,
        'eventos_grid': eventos_grid,
        'programas_grid': programas_grid,
